...
class Angry_Bird:
    ...
    def load_images(self):
        if Angry_Bird.images == None:
            ...
            for name in animation_names:
                Angry_Bird.images[name] = [load_image("./bird/" + name + "%d" % i + ".png") for i in range(1, 6+1)]
            # eagle_2_0 프레임 뒤에 5,4,3,2 순서의 프레임을 추가
            if 'eagle_2_0' in Angry_Bird.images:
                for idx in [4, 3, 2, 1]:
    ...
    def __init__(self, x=None, y=None):
        ...
        self.state = 'eagle_2_0'
        self.ball_count = 0
        self.strength = 5
        self.now_hp = Angry_Bird.max_hp
        if Angry_Bird.hpbar == None:
            Angry_Bird.hpbar = load_image('hpbar.png')
        if Angry_Bird.hpblank == None:
            Angry_Bird.hpblank = load_image('hpblank.png')
    ...
    def draw(self):
        if math.cos(self.dir) > 0:
            Angry_Bird.images[self.state][int(self.frame)].composite_draw(0, 'h', self.x, self.y, 100, 100)
        else:
    ...
    def build_behavior_tree(self): # sequence
        ...
        root = chase_or_patrol = Selector('추적 또는 순찰', shoot_seq, chase_if_boy_nearby, patrol)
